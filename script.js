...
const IS_HOME = !params.has("name") && !params.has("from") && !params.has("msg");
const NAME = (params.get("name") || "Friend").trim().slice(0, 30) || "Friend";
const CUSTOM_MSG = (params.get("msg") || "").slice(0, 200);
...
function buildLink() {
  ...
  // always set name so the link opens the experience, never the home page
  url.searchParams.set("name", homeName.value.trim() || "Friend");
